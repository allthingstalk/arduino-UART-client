...
This sketch is an example sketch to deploy the Grove Pir motion sensor (101020020) to the
...
1. Setup the Arduino hardware
  ...
  - Connect a Grove Pir motion sensor to pin D2 of the Arduino shield
...
#define motionId 2                // Digital sensor is connected to pin D2 on grove shield
...
void setup()
{
  ...
  Device.AddAsset(motionId, "Pir motion sensor", "motion", false, "boolean");   // Create the sensor asset for your device

  delay(1000);                                         // Give the WiFi some time to finish everything
  while(!Device.Subscribe(mqttServer, callback))       // Make sure that we can receive message from the AllThingsTalk IOT developer cloud (MQTT). This stops the http connection
    Serial.println("Retrying");

  pinMode(motionId, INPUT);                            // Initialize the digital pin as an input.
  Device.Send("false", motionId);
  Serial.println("Pir motion sensor is ready for use!");
}
...
void loop() 
{
  bool sensorRead = digitalRead(motionId);                 // Read status Digital Sensor
